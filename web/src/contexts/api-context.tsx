...
interface API {
    ...
    saveDatapoint: (container: string, data: Datapoint) => Promise<Datapoint>;
}
...
class RestAPI implements API {
    ...
    createContainer: (container: Container) => Promise<Container> = (container) =>
        this.doPost<Container>("/container", container);

    getAllDatapoints: (container: string) => Promise<Datapoint[]> = (container) =>
        this.doGet<Datapoint[]>(`/container/${container}`);

    saveDatapoint: (container: string, data: Datapoint) => Promise<Datapoint> = (container, data) =>
        this.doPost<Datapoint>(`/container/${container}/${data.label}`, data)
}

const apiContext = React.createContext<API>(new RestAPI(""));
